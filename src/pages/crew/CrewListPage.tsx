...
const CrewListPage = (props: PageTagProps) => {
    ...
    return (
        <>
            ...
            <main>
                <SelCommunity>
                    <Button color={activeButton === -1 ? "secondary" : "outline-secondary"} block onClick={e => selectCommunityList(-1, crewList)}> 전체<br />({crewList.length}) </Button>&nbsp;
                    <Button color={activeButton === 0 ? "secondary" : "outline-secondary"} block onClick={e => selectCommunityList(0, crewList)}> 1청년부<br />({community1.length}) </Button>&nbsp;
                    <Button color={activeButton === 1 ? "secondary" : "outline-secondary"} block onClick={e => selectCommunityList(1, crewList)}> 2청년부<br />({community2.length}) </Button>&nbsp;
                    <Button color={activeButton === 2 ? "secondary" : "outline-secondary"} block onClick={e => selectCommunityList(2, crewList)}> 3청년부<br />({community3.length}) </Button>&nbsp;
                    <Button color={activeButton === 3 ? "secondary" : "outline-secondary"} block onClick={e => selectCommunityList(3, crewList)}> 신혼BR<br />({communityBR.length}) </Button>
                </SelCommunity>
